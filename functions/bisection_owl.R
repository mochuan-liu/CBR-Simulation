owl_single <- function(K, H, A, P, Y, lambda, sigma){
...
weight_least_square <- function(H, Y, weight){
  cv.res <- try(cv.glmnet(x = H, y = Y, weights = weight, family = 'gaussian'), silent = TRUE)
  cv.lambda <- cv.res$lambda.min
  m.model <- glmnet(x = H, y = Y, weights = weight, lambda = cv.lambda, family = 'gaussian') 
  return(m.model)
}

est_opt_owl_single <- function(out, K_testing = NULL){
  beta <- out$beta %>% matrix(ncol = 1)
  beta0 <- out$beta0
  K_testing <- as.matrix(K_testing)
  return(sign(K_testing%*%beta+beta0))
}

# AOWL
aowl <- function(K_list, H_list, A_list, P_list, Y, lambda_vec, sigma_list){
  ...
  for(t in T:1){
    if(t == T){
      ...
      o <- owl_single(K = K_list[[T]], H = H_list[[T]], A = A_list[[T]], P = P_list[[T]], Y = Y, lambda = lambda_vec[T], sigma = sigma_list[[T]])
      A_est_list[[T]] <- est_opt_owl_single(out = o, K_testing = K_list[[T]])
      ...
      o <- owl_single(K = K_list[[t]], H = H_list[[t]], A = A_list[[t]], P = P_list[[t]], Y = Y_now - Y_aug, lambda = lambda_vec[t], sigma = sigma_list[[t]])
      A_est_list[[t]] <- est_opt_owl_single(out = o, K_testing = K_list[[t]])
...
# OWL
bowl <- function(K_list, H_list, A_list, P_list, Y, lambda_vec, sigma_list){
  ...
  for(t in T:1){
    if(t == T){
      ...
      Y_now <- Y_now*(A_est_list[[t+1]]==A_list[[t+1]])/P_list[[t+1]]
      o <- owl_single(K = K_list[[t]], H = H_list[[t]], A = A_list[[t]], P = P_list[[t]], Y = Y_now, lambda = lambda_vec[t], sigma = sigma_list[[t]])
      A_est_list[[t]] <- est_opt_owl_single(out = o, K_testing = K_list[[t]])
...
owl2 <- function(K_list, H_list, A_list, P_list, Y, lambda_vec, sigma_list, method){
  suppressMessages(require(glmnet))
  
  if(method == 'aowl'){
    return(aowl(K_list, H_list, A_list, P_list, Y, lambda_vec, sigma_list))
  }else{
    return(bowl(K_list, H_list, A_list, P_list, Y, lambda_vec, sigma_list))
  }
}

# Bisection search
bisection_owl <- function(K_list, H_list, A_list, P_list, P, Reward, Risk, tau, 
                          lambda_vec, sigma_list, epsilon, eta, method){
  ...
  res_min <- try(owl2(K_list = K_list, H_list = H_list, A_list = A_list, P_list = P_list, Y = R_min, lambda_vec = lambda_vec, sigma_list = sigma_list, method = method), silent = TRUE)
  res_max <- try(owl2(K_list = K_list, H_list = H_list, A_list = A_list, P_list = P_list, Y = R_max, lambda_vec = lambda_vec, sigma_list = sigma_list, method = method), silent = TRUE)
  ...
  while(gamma_min - gamma_max>=epsilon&n_try <=100){
    ...
    res <- try(owl2(K_list = K_list, H_list = H_list, A_list = A_list, P_list = P_list, Y = R_now, lambda_vec = lambda_vec, sigma_list = sigma_list, method = method), silent = TRUE)
...
# Main function with CV
br2_owl <- function(H_list, A_list, P_list, P, Reward, Risk, tau, kernel = 'gaussian', 
                    lambda_list, sigma_list = NULL,
  ...
  for(k.cv in 1:cv.folds){
    ...
    temp.list$K_list_training <- K_matrix_list(H_list = temp.list$H_list_training, H_testing_list = temp.list$H_list_training, kernel = kernel, sigma_list = sigma_list)
    temp.list$K_list_testing  <- K_matrix_list(H_list = temp.list$H_list_training, H_testing_list = temp.list$H_list_testing,  kernel = kernel, sigma_list = sigma_list)
  ...
  for(i in 1:dim(dat_para)[1]){
    ...
    for(k.cv in 1:cv.folds){
      ...
      K_training <- dat$K_list_training
      K_testing  <- dat$K_list_testing
      
      # Bisection search
      o_search <- try(bisection_owl(K_list = K_training, H_list = H_training_list, A_list = A_training_list, P_list = P_training_list, P = P_training, 
                                    Reward = Reward_training, Risk = Risk_training, tau = tau, 
                                    lambda_vec = lambda_vec_now, sigma_list = sigma_list, epsilon = epsilon, eta = eta, method = method), silent = TRUE)
      if(!is.list(o_search)){
      ...
      opt_training <- est_opt(o_search$res, K_list = K_training)
      opt_testing  <- est_opt(o_search$res, K_list = K_testing)
  ...
  o <- try(bisection_owl(K_list = K_list, H_list = H_list, A_list = A_list, P_list = P_list, P = P,
                         Reward = Reward, Risk = Risk, tau = tau, 
                         lambda_vec = lambda_vec_max, sigma_list = sigma_list, epsilon = epsilon, eta = eta, method = method))
  if(!is.character(o)){
