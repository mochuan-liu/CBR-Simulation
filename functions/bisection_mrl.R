# Bisection for mrl
bisection_mrl <- function(H_list, K_list, A_list, P_list, P, Reward, Risk, tau, 
                           init_type = 'model', kernel = 'gaussian', mu2, lambda_vec, sigma_list = NULL, epsilon, eta){
  ...
  if(init_type == 'model'){
    init_min <- get_reg_init(K_list = K_list, A_list = A_list, Y = R_min,
                             kernel = kernel, sigma_list = sigma_list)
    init_max <- get_reg_init(K_list = K_list, A_list = A_list, Y = R_max,
                             kernel = kernel, sigma_list = sigma_list)
  }
  ...
  res_min <- try(single_dc(K_list = K_list, A_list = A_list, 
                           R = R_min/P, 
                           beta_start = init_min$beta, beta0_start = init_min$beta0, mu2 = mu2, 
                           lambda_vec = lambda_vec, kernel = kernel, sigma_list = sigma_list, type = 'smooth'), silent = TRUE)
  
  # Estimate initial end point (max)
  res_max <- try(single_dc(K_list = K_list, A_list = A_list, 
                           R = R_max/P, 
                           beta_start = init_max$beta, beta0_start = init_max$beta0, mu2 = mu2, 
                           lambda_vec = lambda_vec, kernel = kernel, sigma_list = sigma_list, type = 'smooth'), silent = TRUE)
  ...
  while(gamma_min - gamma_max>=epsilon&n_try<=100){
    ...
    if(init_type == 'model'){
      init <- get_reg_init(K_list = K_list, A_list = A_list, Y = R_now,
                           kernel = kernel, sigma_list = sigma_list)
    }
    
    R_now <- subtract_mean(R_now, H_list[[1]])
    
    res <- try(single_dc(K_list = K_list, A_list = A_list, 
                         R = R_now/P, 
                         beta_start = init$beta, beta0_start = init$beta0, mu2 = mu2, 
                         lambda_vec = lambda_vec, kernel = kernel, sigma_list = sigma_list, type = 'smooth'), silent = TRUE)
...
br2_mrl <- function(H_list, A_list, P_list, P, Reward, Risk, tau, kernel = 'gaussian', 
                    sigma_list = NULL, mu2_list = 10^(-8), lambda_list = list(c(0.01,0.1), c(0.01,0.1)),
                    init_type = 'model', cv.folds = 2, epsilon = 0.001, eta = 0.0001){
  
  T <- length(A_list)
  N <- length(A_list[[1]])

  # compute kernel bandwidth if not provided
  ...
  for(k.cv in 1:cv.folds){
    temp.list <- list(H_list_training = list(), K_list_training = list(), A_list_training = list(), P_list_training = list(), P_training = c(),
                      H_list_testing  = list(), K_list_testing  = list(), A_list_testing  = list(), P_list_testing  = list(), P_testing  = c(),
    ...
    for(t in 1:T){
      ...
      temp.list$K_list_training[[t]] <- kernel_matrix(H = temp.list$H_list_training[[t]], sigma = sigma_list[[t]], kernel = kernel)
      ...
      temp.list$K_list_testing[[t]] <- kernel_matrix(H = temp.list$H_list_training[[t]], H_testing = temp.list$H_list_testing[[t]], sigma = sigma_list[[t]], kernel = kernel)
    }
  ...
  for(i in 1:dim(dat_para)[1]){
    ...
    for(k.cv in 1:cv.folds){
      ...
      K_training_list <- dat$K_list_training
      ...
      K_testing_list <- dat$K_list_testing
      ...
      # Bisection search
      o_search <- try(bisection_mrl(H_list = H_training_list, K_list = K_training_list, A_list = A_training_list, P = P_training, 
                                     Risk = Risk_training, Reward = Reward_training, tau = tau,
                                     kernel = kernel, mu2 = mu2_now, lambda_vec = lambda_vec_now, 
                                     init_type = init_type, sigma_list = sigma_list, epsilon = epsilon, eta = eta), silent = TRUE)
      if(!is.list(o_search)){
      ...
      opt_training <- est_opt(o_search$res, K_list = K_training_list)
      opt_testing  <- est_opt(o_search$res ,K_list = K_testing_list)
  ...
  o <- try(bisection_mrl(H_list = H_list, K_list = K_list, A_list = A_list, P = P, Risk = Risk, Reward = Reward, tau = tau,
                          kernel = kernel, mu2 = mu2_max, lambda_vec = lambda_vec_max, 
                          init_type = init_type, sigma_list = sigma_list, epsilon = epsilon, eta = eta))
  if(!is.character(o)){
...
compute_risk <- function(V, P, A, K_list, out, eta = 0.0001){
  ...
  for(t in 1:T){
    K_t <- K_list[[t]]
    f_t <- cbind(1, K_t) %*% (c(beta0[[t]], beta[[t]]))
    af_t <- f_t*A[[t]]
