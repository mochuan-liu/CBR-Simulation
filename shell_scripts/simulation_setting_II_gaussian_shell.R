...
for(seed in seed_list){
    for(N in c(200,400)){
      ...
      for(method in method_list){
        file_name <- paste(paste('Simulation_II_gaussian', method, N, seed, sep = '_'), '.sh', sep = '')
        file_path <- file.path(working_path, '/shell_scripts/Setting_II_gaussian/')
        ...
        writeLines(c('#!/bin/bash',
                     ...
                     'module add r/4.1.3',
                     '',
                     sprintf('Rscript simulation_setting_II_gaussian.R --seed %s --N %s --method %s', 
                             seed,
                             N,
                             method)), 
                   fileConn, sep = '\n')
