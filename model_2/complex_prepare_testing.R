...
H <- runif(7*N, 0, 1) %>% matrix(ncol = 7) %>% as.data.frame()
colnames(H) <- sapply(1:dim(H)[2], FUN = function(x) paste('X', x, sep = ''))

X8_based <- runif(N, 0.5, 1)
...
O <- searching_opt(H = H, A1 = A1, fun_y = y, fun_r = r, tau_list = c(seq(1, 2 , 0.1), 100), gamma_list = seq(0, 20, 0.02), epsilon = 0.01)

saveRDS(list(O = O, H = H, H1 = H[,c(1:8)], H2 = cbind(H, A1), A1 = A1, A2 = A2,
             Y = Y, R = R), file.path(work_path, 'data/complex_testing_2.rds'))
